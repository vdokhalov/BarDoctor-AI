...
function safeFileName(value: string): string {
  ...
  return (clean || "purchase-document.jpg").slice(0, 180);
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!hasPermission(account, "inventory.manage")) {
    return noStore(
      { ok: false, code: "ACCESS_DENIED", error: "У вас нет права загружать закупочные документы" },
      403,
    );
  }
  ...
  try {
    ...
    return noStore({ ok: false, error: "Не удалось прочитать фотографию документа" }, 400);
  }
  const file = form.get("file");
  if (!isUploadFile(file)) {
    return noStore({ ok: false, error: "Выберите фотографию документа" }, 400);
  }
  if (file.size <= 0 || file.size > MAX_STAGED_IMAGE_BYTES) {
    return noStore(
      { ok: false, error: "Фото должно быть не больше 700 КБ после подготовки" },
      413,
    );
  }
  ...
  if (!bucket) {
    return noStore({ ok: false, error: "Хранилище документов временно недоступно" }, 503);
  }
  const id = crypto.randomUUID();
  const source = form.get("source") === "camera" ? "camera" : "gallery";
  await bucket.put(`purchases/${account.id}/${id}`, bytes, {
