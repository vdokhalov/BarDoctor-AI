...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!hasPermission(account, "settings.manage")) {
    return noStore({ ok: false, code: "ACCESS_DENIED", error: "Нет права изменять логотип заведения" }, 403);
  }
  ...
  try {
    ...
    return noStore({ ok: false, error: "Не удалось прочитать логотип" }, 400);
  }
  const file = form.get("file");
  if (!isUploadFile(file)) return noStore({ ok: false, error: "Выберите логотип заведения" }, 400);
  if (file.size <= 0 || file.size > MAX_LOGO_BYTES) {
    return noStore({ ok: false, error: "Логотип должен быть не больше 1 МБ после подготовки" }, 413);
  }
  ...
  if (!storage) return noStore({ ok: false, error: "Хранилище логотипов временно недоступно" }, 503);

  const id = crypto.randomUUID();
  await storage.put(`venues/${account.id}/logos/${id}`, bytes, {
    httpMetadata: { contentType: mimeType },
    customMetadata: {
      uploadedAt: new Date().toISOString(),
      purpose: "venue-logo",
      venueId: String(account.venueId),
    },
  });
  return noStore({ ok: true, logo: { id, mimeType, size: bytes.byteLength } });
}
