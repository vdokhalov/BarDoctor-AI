...
## Slice 2 — opening documents and CSV onboarding

Implemented separate opening_balance movements and immutable document snapshots, distinct optional opening valuation/source, no artificial receipts or last-purchase value. Pure preview and selected-row all-or-nothing confirmation use the existing D1 CAS transaction with audit and permission/venue/month guards. Idempotency records are independent of movement retention. Existing quantities, movement/document history, and missing authoritative assortment with retained history require review; no automatic backfill. Opening documents are excluded from bulk bootstrap SQL and read via the scoped endpoint. A 4 MB history capacity guard rejects additional writes instead of deleting old idempotency records.

Manual and CSV UTF-8 flows share the same command. CSV is the initial supported bulk format (the requested CSV/XLSX alternatives); XLSX parsing is not added. Limits: 500 rows, 20 columns, 512 KB, no evaluated formulas. Preview exposes each row's errors and selected/skipped counts. Taxonomy accepts unique exact names within the selected parent or stable IDs. Optional package metadata and opening quantity/cost are independent. Existing item selection uses its explicit ID, not a guessed name link. New entry is available from Warehouse without a global redesign.
