...
function currencyLabel(value: string): string {
  ...
  return ({ MDL: "лей", RUB: "₽", EUR: "€", USD: "$", RON: "lei" } as Record<string, string>)[code] ?? code;
}
...
function financePeriods(current: JsonRecord, closed: JsonRecord, generatedAt: Date) {
  const currentMonthKey = text(current.monthKey, generatedAt.toISOString().slice(0, 7), 7);
  ...
  const currentPeriod: MetricPeriod | null = currentRevenue !== null || currentExpenses !== null
    ? {
        ...
        endDate: validDate(current.endDate) ?? generatedAt.toISOString().slice(0, 10),
...
function financeComponent(
  ...
  if (profitDelta !== null) {
    ...
    gaps.push("Нет второго закрытого периода для оценки динамики");
  }
...
function healthFromComponents(
  ...
  return {
    ...
    label: score === null
      ...
      : financeScore !== null && financeScore < 35
        ? "critical" as const
        : score < 45 ? "critical" as const : score < 70 ? "attention" as const : "healthy" as const,
    confidencePercent,
...
export function buildBusinessIntelligence(input: BusinessIntelligenceInput): AIDoctorIntelligence {
  ...
  const periodContext = financePeriods(input.currentFinancialPeriod ?? {}, input.latestClosedMonth ?? {}, now);
  const components = [
