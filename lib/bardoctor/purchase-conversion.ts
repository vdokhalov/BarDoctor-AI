...
export function preparePurchaseConversions(document: PurchaseDocument, rawDocument: unknown, assortment: unknown, preserveSnapshots = false) {
  ...
  const items = document.items.map((item, index) => {
    ...
    if (preserveSnapshots && raw.purchaseConversion != null) {
      ...
      if (!snapshot) issues.push({ id: item.id, error: "Историческая конверсия требует проверки." });
      return snapshot ? { ...item, purchaseConversion: snapshot } : item;
    }
    const reference = String(raw.purchaseProductKey ?? raw.nomenclatureId ?? "");
