...
function summariseRevenue(request: JsonRecord, sources: VenueAIContextSources, now: Date) {
  ...
  const currentReceipts = sum(current, receiptsOf);
  const currentGuests = sum(current, guestsOf);
...
function summarisePurchasesAndInventory(sources: VenueAIContextSources, now: Date) {
  ...
  for (const { item } of purchaseItems) {
    ...
    current.spend += number(item.lineTotal) ?? 0;
    current.quantity += number(item.quantity) ?? 0;
    current.lastPrice = number(item.unitPrice) ?? current.lastPrice;
...
function summariseSeasonality(request: JsonRecord, sources: VenueAIContextSources, now: Date) {
...
export function buildVenueAIContextFromSources(
  ...
  const seasonality = summariseSeasonality(request, sources, now);
