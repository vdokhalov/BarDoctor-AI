-- User-confirmed recovery of the exact archived venue #3287.
...
SELECT pa.`account_id`, 'venue.restore_confirmed', 'venue', '3287',
  '{"status":"archived","workspaceId":3168,"dataAccountId":15,"createdByAccountId":15}',
  '{"status":"active","workspaceId":3168,"dataAccountId":15,"createdByAccountId":15}',
  'success',
  'User explicitly authorized restoring archived venue 3287; canonical owner memberships verified',
  'restore-venue-3287-20260824', CURRENT_TIMESTAMP
...
WHERE pa.`status` = 'active'
  AND EXISTS (
    ...
    WHERE v.`id` = 3287
      ...
      AND v.`workspace_id` = 3168
      AND v.`data_account_id` = 15
      AND v.`created_by_account_id` = 15
      AND w.`status` = 'active'
      AND w.`created_by_account_id` = 15
      AND a.`account_kind` = 'user'
      AND a.`owns_venue` = 1
      AND EXISTS (
        SELECT 1 FROM `venue_memberships` vm
        WHERE vm.`venue_id` = v.`id`
          AND vm.`account_id` = 15
          AND vm.`role` = 'owner'
  ...
  AND NOT EXISTS (
    ...
    WHERE existing.`request_id` = 'restore-venue-3287-20260824'
  )
...
WHERE `id` = 3287
  AND `status` = 'archived'
  AND `workspace_id` = 3168
  AND `data_account_id` = 15
  AND `created_by_account_id` = 15
  AND EXISTS (
