(function () {
  ...
  window.fetch = function (input, init) {
    ...
    if (url.indexOf("/api/auth/bootstrap") >= 0) {
      return Promise.resolve(new Response(JSON.stringify({ ok: true, email: email, userId: "qa-assortment-user", token: "qa-local-token", firstName: "QA", lastName: "Assortment", phone: null, role: activeRole, permissions: permissions, activeVenueId: venueId, activeWorkspaceId: "qa-assortment-workspace", activeVenueIsPrimary: venueId === 501, canCreateVenues: true, venues: venueRows }), { status: 200, headers: jsonHeaders }));
    }
