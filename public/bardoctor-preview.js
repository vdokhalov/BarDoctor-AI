(async function () {
  ...
  function injectCompetitorsEntry() {
    ...
    entry.setAttribute("data-bd-competitors-entry", "");
    entry.innerHTML = [
      '<div class="bd-competitors-entry-head">',
        '<span class="bd-competitors-entry-icon" aria-hidden="true"><i></i><b></b></span>',
        '<span class="bd-competitors-entry-title">',
          '<small>РЫНОК ВОКРУГ ВАС</small>',
          '<strong>Конкуренты</strong>',
        '</span>',
        '<span class="bd-competitors-entry-badge" data-bd-competitors-badge>Загрузка…</span>',
      '</div>',
      '<p class="bd-competitors-entry-status" data-bd-competitors-status aria-live="polite">Проверяю сохранённый список конкурентов.</p>',
      '<div class="bd-competitors-entry-stats">',
        '<span><strong data-bd-competitors-confirmed>—</strong><small>подтверждено</small></span>',
        '<span><strong data-bd-competitors-candidates>—</strong><small>требуют проверки</small></span>',
      '</div>',
      '<div class="bd-competitors-entry-names" data-bd-competitors-names></div>',
      '<a class="bd-competitors-entry-action" href="/market#competitors-card">',
        '<span data-bd-competitors-action>Открыть список</span><b aria-hidden="true">→</b>',
      '</a>',
    ].join("");

    aiCard.parentNode.insertBefore(entry, aiCard);
    var actionLink = entry.querySelector(".bd-competitors-entry-action");
    if (actionLink) {
    ...
    fetch("/api/competitors/me")
      .then(function (response) { return response.json(); })
      .then(function (result) {
        if (!result || result.ok === false) throw new Error("competitors unavailable");
        var competitors = Array.isArray(result.competitors) ? result.competitors : [];
        var confirmed = typeof result.confirmedCount === "number"
          ? result.confirmedCount
          : competitors.filter(function (item) { return item && item.confirmed === true; }).length;
        var candidates = typeof result.candidateCount === "number"
          ? result.candidateCount
          : Math.max(0, competitors.length - confirmed);
        var badge = entry.querySelector("[data-bd-competitors-badge]");
        var status = entry.querySelector("[data-bd-competitors-status]");
        var confirmedNode = entry.querySelector("[data-bd-competitors-confirmed]");
        var candidatesNode = entry.querySelector("[data-bd-competitors-candidates]");
        var names = entry.querySelector("[data-bd-competitors-names]");
        var action = entry.querySelector("[data-bd-competitors-action]");
        if (badge) badge.textContent = competitors.length ? competitors.length + " найдено" : "Список пуст";
        if (confirmedNode) confirmedNode.textContent = String(confirmed);
        if (candidatesNode) candidatesNode.textContent = String(candidates);
        if (action) action.textContent = competitors.length ? "Открыть список" : "Найти конкурентов";
        if (status) {
          status.textContent = !competitors.length
            ? "Список ещё не сформирован. Проведите анализ рынка — BarDoctor найдёт реальные заведения рядом."
            : confirmed > 0
              ? "AI сравнивает заведение только с подтверждёнными конкурентами."
              : "Заведения найдены, но ни одно ещё не подтверждено как реальный конкурент.";
        }
        if (names) {
          while (names.firstChild) names.removeChild(names.firstChild);
          var ordered = competitors.slice().sort(function (left, right) {
            return Number(Boolean(right && right.confirmed)) - Number(Boolean(left && left.confirmed));
          });
          ordered.slice(0, 3).forEach(function (competitor) {
            var name = document.createElement("span");
            name.textContent = (competitor.confirmed ? "✓ " : "") + competitor.name;
            if (competitor.confirmed) name.className = "confirmed";
            names.appendChild(name);
          });
          if (competitors.length > 3) {
            var more = document.createElement("span");
            more.textContent = "+" + (competitors.length - 3);
      ...
      .catch(function () {
        ...
        if (status) status.textContent = "Не удалось загрузить список. Откройте раздел и повторите попытку.";
      });
  }
  ...
  function injectOpportunityEntry() {
    ...
    entry.innerHTML = [
      '<div class="bd-opportunity-entry-head">',
        '<span class="bd-opportunity-entry-icon" aria-hidden="true"><i></i><b></b><em></em></span>',
        '<span class="bd-opportunity-entry-title"><small>ПОВОДЫ ДЛЯ РОСТА</small><strong>Календарь возможностей</strong></span>',
        '<span class="bd-opportunity-entry-badge" data-bd-opportunity-badge>Загрузка…</span>',
      '</div>',
      '<p class="bd-opportunity-entry-status" data-bd-opportunity-status>Проверяю ближайшие праздники, матчи, концерты и городские события.</p>',
      '<div class="bd-opportunity-entry-main">',
        '<span class="bd-opportunity-entry-next"><small>БЛИЖАЙШАЯ ВОЗМОЖНОСТЬ</small><strong data-bd-opportunity-next>—</strong></span>',
        '<span class="bd-opportunity-entry-score"><strong data-bd-opportunity-score>—</strong><small>из 100</small></span>',
      '</div>',
      '<a class="bd-opportunity-entry-action" href="/opportunities"><span data-bd-opportunity-action>Открыть календарь</span><b aria-hidden="true">→</b></a>',
    ].join("");
    ...
    fetch("/api/opportunities")
      .then(function (response) { return response.json(); })
      .then(function (result) {
        if (!result || result.ok === false) throw new Error("calendar unavailable");
        var events = result.calendar && Array.isArray(result.calendar.events)
          ? result.calendar.events.filter(function (event) { return event && event.decision !== "dismissed"; })
          : [];
        var high = events.filter(function (event) { return event.potentialLabel === "high"; });
        var best = events.slice().sort(function (left, right) {
          return Number(right.potentialScore || 0) - Number(left.potentialScore || 0)
            || String(left.startDate || "").localeCompare(String(right.startDate || ""));
        })[0];
        var badge = entry.querySelector("[data-bd-opportunity-badge]");
        var status = entry.querySelector("[data-bd-opportunity-status]");
        var next = entry.querySelector("[data-bd-opportunity-next]");
        var score = entry.querySelector("[data-bd-opportunity-score]");
        var action = entry.querySelector("[data-bd-opportunity-action]");
        if (badge) badge.textContent = events.length ? high.length + " важных" : "Нужен поиск";
        if (status) status.textContent = events.length
          ? "BarDoctor оценил события по аудитории, графику, близости и коммерческому потенциалу."
          : "Календарь ещё не собран. Город и формат будут взяты из профиля автоматически.";
        if (next) next.textContent = best ? best.title : "Запустите первый поиск";
        if (score) score.textContent = best ? String(best.potentialScore) : "—";
        if (action) action.textContent = events.length ? "Открыть календарь" : "Найти возможности";
      })
      .catch(function () {
        var badge = entry.querySelector("[data-bd-opportunity-badge]");
        var status = entry.querySelector("[data-bd-opportunity-status]");
        if (badge) badge.textContent = "Нет данных";
        if (status) status.textContent = "Откройте календарь и повторите поиск возможностей.";
      });
  }
  ...
  function loadApplication() {
    ...
    script.type = "module";
    script.src = "/assets/index-BQGspy0I.js?v=20260821-inventory-reconciliation-v224-user-display-units-v236-purchase-units-v237-collapsed-tree-v239-accounting-currency-v243-warehouse-valuation-v244-inventory-workflow-v245-inventory-layer-v246-20260823-auth-login-v248-20260823-existing-venue-gate-v249-20260823-embedded-login-transition-v250-20260823-venue-setup-boundary-v251-20260823-inventory-scope-hierarchy-v256-20260823-tech-card-reconciliation-v257-20260823-tech-card-semantic-matching-v258-20260823-tech-card-entity-resolution-v259-20260826-tech-card-consistency-v299a-20260826-invoice-recognition-v2-20260824-canonical-supplier-v260-20260824-auth-bootstrap-state-v274-20260825-profile-v280a-20260825-profile-v281-20260825-profile-v282-20260825-business-health-v284-20260826-venue-identity-v297-20260826-menu-sale-size-v298-20260826-invoice-mapping-name-integrity-v296";
