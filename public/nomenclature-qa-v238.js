(function () {
  ...
  window.fetch = function (input, init) {
    ...
    if (url.indexOf('/api/auth/bootstrap') >= 0) return Promise.resolve(new Response(JSON.stringify({ ok: true, email: email, userId: 'qa-nomenclature-user', token: 'qa-local-token', firstName: 'QA', lastName: 'Nomenclature', role: 'owner', permissions: permissions, activeVenueId: venueId, activeWorkspaceId: 'qa-nomenclature-workspace', activeVenueIsPrimary: venueId === 501, canCreateVenues: true, venues: venues }), { status: 200, headers: headers }));
