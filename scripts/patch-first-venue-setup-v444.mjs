...
if (!bundle.includes('bd-first-venue-setup-v444')) {
    ...
    setup = replace(setup, 'await t(A),_.length>0&&n(_),m(!0),setTimeout(()=>e("/home"),2e3)', `bdSubmitting.current=true;bdSetSaving(true);try{await t(A);if(_.length>0)n(_);sessionStorage.removeItem(bdDraftKey);m(true);e('/home')}catch(error){bdSetError(!error?.message||/failed to fetch|load failed|network/i.test(error.message)?'Не удалось связаться с сервером. Проверьте соединение и повторите попытку.':error.message)}finally{bdSubmitting.current=false;bdSetSaving(false)}`);
