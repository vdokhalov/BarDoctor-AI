...
const cacheToken = "20260906-home-reviews-lifecycle-v414";
let source = fs.readFileSync(bundlePath, "utf8");
const homeReviewsHookV414 = 'function bdHomeReviewsAuthReadyV414(){return window.__bdBootstrapPending!==!0&&window.__bdAuthBootstrapV274?.state==="ready"}function bdUseHomeReviewsV409(e,t){const[n,r]=S.useState({status:"loading",data:null});return S.useEffect(()=>{let a=!0,s=0;const l=()=>{const e=++s;if(r({status:"loading",data:null}),!t||!bdHomeReviewsAuthReadyV414())return;bdFetchHomeReviewsV409().then(t=>{a&&e===s&&r(t)}).catch(t=>{a&&e===s&&r({status:t?.status===403?"forbidden":"error",data:null,error:t?.message||"Отзывы временно недоступны"})})},u=()=>{bdHomeReviewsCacheV409.clear(),bdHomeReviewsPromisesV409.clear(),l()},d=()=>l();return l(),window.addEventListener("bd:active-venue-changed",u),window.addEventListener("bd:bootstrap-complete",d),()=>{a=!1,s++,window.removeEventListener("bd:active-venue-changed",u),window.removeEventListener("bd:bootstrap-complete",d)}},[e,t]),n}';
...
const homeReviewsAuthHookStart = source.indexOf("function bdHomeReviewsAuthReadyV414(");
...
if (source.slice(homeReviewsHookStart, homeReviewsHookEnd) !== homeReviewsHookV414) {
  source = source.slice(0, homeReviewsHookStart) + homeReviewsHookV414 + source.slice(homeReviewsHookEnd);
}
