...
function bdPurchaseReview({draft:e,suppliers:t,onChange:n,onCancel:r,onConfirm:a,onAddSupplier:o,saving:s}){
  ...
  const f=()=>{const id=crypto.randomUUID();l("items",[...e.items,{id,name:"",quantity:1,unit:"шт.",quantityMode:"count",packageSize:"1 шт.",unitPrice:"",lineTotal:"",costStatus:"UNKNOWN",category:e.expenseCategory||"products",confidence:1}]);bdSetActiveLineV357(id);bdSetLineViewV357("all")};
  ...
  const bdLineCostKnownV414=line=>line.costStatus==="KNOWN_ZERO"||line.costStatus==="KNOWN"||Number(line.unitPrice)>0||Number(line.lineTotal)>0;
  const bdLinesReady=e.documentType==="price_list"?e.items.length>0&&e.items.every(line=>String(line.name||"").trim()&&bdLineCostKnownV414(line)):e.items.length>0&&e.items.every(line=>(e.source==="manual"||(!line.requiresReview&&Boolean(line.purchaseProductKey||line.nomenclatureId)))&&String(line.name||"").trim()&&Number(line.quantity)>0&&bdLineCostKnownV414(line));
  const bdTotalMismatchV357=e.documentType!=="price_list"&&Math.abs(Number(e.total||0)-d)>.02;
  const bdCanPostV357=bdSupplierReady&&bdLinesReady&&(e.documentType==="price_list"||!bdTotalMismatchV357);
  const bdBlockingTextV357=!bdSupplierReady?"Выберите поставщика":!e.items.length?"Добавьте хотя бы одну позицию":bdAttentionLinesV357.length?"Сопоставьте "+bdAttentionLinesV357.length+" "+bdProcPluralV168(bdAttentionLinesV357.length,"позицию","позиции","позиций"):!bdLinesReady?"Укажите стоимость каждой позиции":bdTotalMismatchV357?"Сверьте итог с суммой позиций":"Готово к проведению";
  ...
  return i.jsx("div",{className:"bd-procurement-sheet-backdrop",children:i.jsxs("section",{className:"bd-procurement-sheet bd-receiving-workspace-v357",role:"dialog","aria-modal":!0,"aria-label":e.status==="confirmed"?"Редактировать приход":"Проверка прихода",children:[
    ...
    i.jsxs("footer",{className:"bd-procurement-sheet-actions bd-receiving-actions-v357",children:[i.jsx("button",{type:"button",className:"bd-procurement-secondary",disabled:s,onClick:r,children:e.status==="confirmed"?"Закрыть":"Не сохранять"}),i.jsxs("div",{children:[i.jsx("small",{children:bdCanPostV357?e.status==="confirmed"?"Изменения будут пересчитаны":"Склад и себестоимость обновятся после проведения":bdBlockingTextV357}),i.jsx("button",{type:"button",className:"bd-procurement-primary",disabled:s||!bdCanPostV357,onClick:a,children:s?"Сохраняю…":e.status==="confirmed"?"Сохранить изменения":e.documentType==="price_list"?"Сохранить прайс":"Провести приход"})]})]})
  ]})})
}
`;
