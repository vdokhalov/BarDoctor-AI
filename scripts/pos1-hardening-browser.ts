...
try{for(const profile of [{name:"mobile",width:390,height:844},{name:"tablet",width:820,height:1000},{name:"desktop",width:1280,height:800}]){
  const r=await lifecycleRuntime({events:"./app/api/sales-events/route",documents:"./app/api/sales-batches/route",restaurantMe:"./app/api/restaurants/me/route",store:"./app/api/store/route",storeKey:"./app/api/store/[key]/route",usersMe:"./app/api/users/me/route"});
  ...
  const structure={sections:[{id:"bar",name:"Бар"},{id:"kitchen",name:"Кухня"},{id:"hookah",name:"Кальяны"}],categories:[{id:"water",name:"Безалкогольные напитки",parentId:"bar"},{id:"food",name:"Продукты",parentId:"kitchen"},{id:"tobacco",name:"Табак",parentId:"hookah"}],subcategories:[]};
  const menu=[{id:"water",name:"QA Вода",sectionId:"bar",taxonomyCategoryId:"water"},{id:"service",name:"QA Сервис",sectionId:"kitchen",taxonomyCategoryId:"food"},{id:"hookah",name:"QA Кальян",sectionId:"hookah",taxonomyCategoryId:"tobacco"},...Array.from({length:14},(_,i)=>({id:"long-"+i,name:"ОченьДлинноеНазваниеБезПробелов".repeat(3)+" "+i,sectionId:i%2?"kitchen":"bar",taxonomyCategoryId:i%2?"food":"water"}))].map(item=>({...item,venueId:venue,department:"bar",category:"Без подраздела",type:"service",consumptionMode:"NONE",salePrice:30,currency:"PMR_RUB",active:true}));
  put("bd_assortment_v1",{nomenclatureStructure:structure,menuItems:menu});
  let failure:"none"|"before"|"after"="none",postCalls=0;
  const server=createServer(async(req,res)=>{try{
    ...
    if(url.pathname==="/api/sales-events"||url.pathname==="/api/sales-batches"){
      ...
      if(action==="post"){postCalls++;if(failure==="before"){failure="none";res.destroy();return;}}
  ...
  try{
    ...
    await page.locator('[data-add="service"]').click();failure="after";await page.locator("#pay").evaluate((button:HTMLButtonElement)=>{button.click();button.click();});await page.locator("#notice").filter({hasText:/Заказ сохранён для безопасного повтора/}).waitFor();await page.reload();await page.locator("#receipt").waitFor();assert.equal(get("bd_sales_events_v1").length,2);assert.equal(get("bd_sales_events_v1")[1].payments[0].method,"CASH");assert.equal(postCalls,2,"double click and lost response must not add another app POST");
    assert.deepEqual(get("bd_stock_movements"),[],"NONE has no stock consumption");
    const event=get("bd_sales_events_v1")[1];await page.goto(base+"/sales-import?embedded=1&venue="+venue+"&batch="+encodeURIComponent(event.id));await page.locator("#editor-dialog[open]").waitFor();assert.equal(await page.locator(".batch-row").count(),2);assert.match(await page.locator("#editor-body").innerText(),/QA Сервис/);assert.match(await page.locator("#editor-body").innerText(),/30.*руб. ПМР/);assert.equal(await page.locator("#editor-footer button").count(),0);assert.equal(await page.locator(".pos-event-summary strong").evaluate(node=>node.getBoundingClientRect().height < 55),true,"receipt revenue is readable on one line");await page.screenshot({path:"outputs/pos1-hardening/"+profile.name+"-document.png"});
    ...
    assert.deepEqual(errors,[]);report.push({profile:profile.name,viewport:profile.width,events:2,revenue:60,stockMovements:0,postCalls,draftReload:true,accountIsolation:true,venueIsolation:true,shiftIsolation:true,crossTabConflict:true,navigation:true,retry:true,document:true});
  }catch(error){console.error({profile:profile.name,url:page.url(),notice:await page.locator("#notice").textContent().catch(()=>""),postCalls,events:(get("bd_sales_events_v1")||[]).length});await page.screenshot({path:"outputs/pos1-hardening/failure.png"}).catch(()=>{});throw error;}finally{await context.close();await new Promise<void>(done=>server.close(()=>done()));r.close();}
