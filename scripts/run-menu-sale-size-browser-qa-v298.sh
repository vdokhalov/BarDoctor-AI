...
qa_log="$(mktemp /tmp/bardoctor-menu-sale-size-qa.XXXXXX.log)"
...
if [[ "$qa_ready" != "1" ]]; then
  tail -n 80 "$qa_log"
  echo "Local menu sale size QA server did not become ready" >&2
  exit 1
fi

BD_QA_BASE_URL="http://127.0.0.1:${qa_port}" \
TMPDIR="${BD_QA_TMPDIR:-/tmp}" \
node scripts/menu-sale-size-browser-qa-v298.cjs
