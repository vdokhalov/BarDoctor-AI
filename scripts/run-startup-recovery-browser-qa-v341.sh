...
qa_port="${BD_QA_PORT:-4176}"
qa_log="$(mktemp /tmp/bardoctor-startup-recovery-qa.XXXXXX.log)"
...
if [[ "$qa_ready" != "1" ]]; then
  tail -n 80 "$qa_log"
  echo "Local startup recovery QA server did not become ready" >&2
  exit 1
fi

BD_QA_BASE_URL="http://127.0.0.1:${qa_port}" \
TMPDIR="${BD_QA_TMPDIR:-/tmp}" \
node scripts/startup-recovery-browser-qa-v341.cjs
