...
test("Procurement v168 is one four-tab management module", async () => {
  ...
  assert.match(procurementFragment, /Сканировать/);
  assert.match(procurementFragment, /Импорт файла/);
...
test("Procurement list uses operational states and item-level review", async () => {
  ...
  assert.match(procurementFragment, /Предполагаемый способ/);
  assert.match(procurementFragment, /не подтверждает факт оплаты/);
  assert.match(purchaseReview, /label:"Предполагаемый способ"/);
...
test("canonical purchase lifecycle separates documents, payments and reversals", async () => {
  ...
  assert.doesNotMatch(confirm, /hasPermission\(account, "expenses\.create"\)/);
