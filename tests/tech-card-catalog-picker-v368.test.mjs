...
const pickerStart = bundle.indexOf("bd-tech-card-search-ux-v375");
const pickerEnd = bundle.indexOf("const bdCatIngredientMatchV259=", pickerStart);
const picker = bundle.slice(pickerStart, pickerEnd);

test("tech-card picker is valid and searches the complete authoritative catalogue", () => {
  ...
  assert.match(bundle, /tokens\.every\(token=>bdTechProductDocumentV375\(item,tree\)\.includes\(token\)\)/);
...
test("tech-card picker exposes the full taxonomy hierarchy and grouped results", () => {
  for (const label of ["Раздел", "Категория", "Подкатегория", "Все разделы", "Все категории", "Все подкатегории", "Поиск по всему справочнику"]) {
    assert.match(bundle, new RegExp(label));
  }
  assert.match(bundle, /bdTechTaxonomyPathV375/);
  assert.match(bundle, /bd-tech-card-groups-v375/);
  assert.match(css, /\.bd-tech-card-taxonomy-v375/);
  assert.match(css, /grid-template-columns:1fr/);
});

test("tech-card picker assets are invalidated in every application shell", () => {
  for (const shell of shells) {
    assert.match(shell, /index-BQGspy0I\.js\?v=[^"']*bd-tech-card-search-ux-v375/);
  }
  for (const shell of cssShells) assert.match(shell, /catalog\.css\?v=[^"']*bd-tech-card-search-ux-v375/);
});
