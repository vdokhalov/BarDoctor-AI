...
test("root entry with an existing venue releases splash without reload", {timeout:120000}, async()=>{
  ...
  try{for(const viewport of [{width:390,height:844},{width:1280,height:720}]){
    for(const entry of ["/","/home?venue=901"]){
      ...
      try{
        ...
        await page.locator('[data-bd-home-page]').waitFor({state:"visible",timeout:15000});
        await page.locator('[data-bd-static-startup="v201"]').waitFor({state:"detached",timeout:5000});
        ...
        assert.equal(documents,1,"handoff must not require reload");
