...
test("warehouse write-offs use the canonical multi-item document workflow", async () => {
  ...
  assert.match(bundle, /function x\(\)\{a\(bdWarehouseNavigationUrlV247\(\{writeoff:null,tab:"writeoffs"\}\),\{replace:!0\}\);window\.dispatchEvent\(new PopStateEvent\("popstate",\{state:window\.history\.state\}\)\)\}/);
